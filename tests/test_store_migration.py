...
def test_initialize_schema_migrates_existing_merge_requests_table():
    """A store created before opened_at/labels existed gains them on initialize_schema, and the
    upsert (which references those columns) then succeeds instead of hitting a binder error."""
    ...
    assert "opened_at" in cols and "labels" in cols
    # pre-existing row has NULLs for the new columns; the crawl backfills on the next sync
    assert conn.execute("SELECT opened_at, labels FROM merge_requests WHERE id = 1").fetchone() == (None, None)
    ...
    store.upsert_merge_requests(conn, [store.MergeRequestRow(
        ...
        labels=["pe:iac-request"], web_url="u", fetched_at=datetime(2026, 7, 28, 0, 0, 0))])
